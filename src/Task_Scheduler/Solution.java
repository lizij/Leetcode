...
public class Solution {
    public int leastInterval(char[] tasks, int n) {
        int[] task = new int[26];
        for (char c: tasks) task[c - 'A']++;
        Arrays.sort(task);
        int max = task[25], idle = (max - 1) * n;
        for (int i = 24; i >= 0 && task[i] > 0; i--) {
            idle -= Math.min(task[i], max - 1);
        }
